...
time, the ensemble member carry only one value, we can rather simply compute the total correlatio between all the 
ensemble members. This will be much harder when handling all the grid. We will probably start by computing the 
correlation matrix. """
...
def process_files(
        keys: list[tuple], fileDate: str
) -> None:
    global arraysList, tpSet
    
    forecast = xr.concat(
        [xr.open_dataarray(tpSet.pathsToFiles[keys[0]]).sel(latitude=lats, longitude=longs).mean(
            ["latitude", "longitude"]
        ),
            xr.open_dataarray(tpSet.pathsToFiles[keys[1]]).sel(latitude=lats, longitude=longs).mean(
                ["latitude", "longitude"]
            )],
        dim="time"
    ).expand_dims({"hdate":[int("".join(fileDate.split('-')))]})
    hindcast = xr.concat(
        [xr.open_dataarray(tpSet.pathsToFiles[keys[2]]).sel(latitude=lats, longitude=longs).mean(
            ["latitude", "longitude"]
        ),
            xr.open_dataarray(tpSet.pathsToFiles[keys[3]]).sel(latitude=lats, longitude=longs).mean(
                ["latitude", "longitude"]
            )],
        dim="time"
    )
    ...
    data['time'] = np.array(
        [
            (day.astype('datetime64[D]').astype(int) - fileDate) for day in data['time'].values
        ]
    )
    ...
    for hdate in data['hdate'].values:
        arraysList.append(data.sel(hdate=hdate))
    data.close()
...
for date in fileDates:
    keys4 = [('forecast', hResName + date),
             ('forecast', lResName + date),
             ('hindcast', hResName + date),
             ('hindcast', lResName + date)]
    if np.any([not (key in tpSet.fileList) for key in keys4]):
        raise "The dataset is uncomplete"
    else:
        process_files(keys4, date)
