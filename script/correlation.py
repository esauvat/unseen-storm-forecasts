...
def process_4_files(
        keys: list[tuple[str, str]], fileDate: str, tpSet: Weatherset,
    ...
    def concat_over_time(
            data1: xr.DataArray, data2: xr.DataArray
    ) -> xr.DataArray:
        ...
        res = xr.concat(
            [data1.sel(latitude=lats, longitude=longs).mean(
                dim=["latitude", "longitude"]),
                data2.sel(latitude=lats, longitude=longs).mean(
                    dim=["latitude", "longitude"])], dim="time")
        return res
    ...
    data.coords['time'] = np.array(
        [(wd.date_as_int(datetime) - intFileDate) for datetime in
...
def hans_area_avg_correlation(
        tpSet: Weatherset, treatmentType: str | None = "daily",
    ...
    corr_distrib = xr.apply_ufunc(
        ...
        wd.pears_distrib, data, input_core_dims=[["number", "obs"]],
        output_core_dims=[["correlations"]], vectorize=True
        # Vectorize to pass 2-D arrays to np.corrcoef
    )
    ...
    ensemble_corr = xr.apply_ufunc(
        wd.pears, data, input_core_dims=[["number"]], output_core_dims=[[]],
        vectorize=True)

    name = (
            's2s-HA_avg-' + resolution + '-ensemble_correlation-' +
            treatmentType)
    path = storingDir + name + '.nc'
