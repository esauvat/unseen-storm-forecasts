""" Script to plot the mean maximum on a large area each year and raise
values over a certain threshold """
...
data = xr.open_dataarray(
    tpSet.compute['continuous_hans-area-avg_' + tpSet.resolution + '_daily']
)
...
def sort_monthly_max(arr: xr.DataArray):
    """ Get the maximum for each month """
    months = np.arange(
        np.datetime64('1941'), np.datetime64('2025'), np.timedelta64(1, 'M')
    )
    res = xr.DataArray(
        np.full((len(alphaMonths), len(years)), None), dims=("months", "years"),
    ...
    for m in months:
        ...
        val = arr.sel(
            time=slice(
                m, m + np.timedelta64(1, 'M') - np.timedelta64(1, 'D')
            )
        ).max()
        res[mIdx, yIdx] = val
    return res
