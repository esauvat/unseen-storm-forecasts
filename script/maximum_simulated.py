...
def annual():
    ...
    if name in tpSet.compute.keys():
        ...
        path = '/nird/projects/NS9873K/emile/unseen-storm-forecasts/weathersets/results/' + name + '.nc'
        wd.map_of_max(data=tpSet, title=title, years=years, splitYears=True, dir=mapDir).to_netcdf(path)
        tpSet.compute[name] = path
...
def monthly():
    ...
    if name in tpSet.compute.keys():
        ...
        path = '/nird/projects/NS9873K/emile/unseen-storm-forecasts/weathersets/results/' + name + '.nc'
        wd.map_of_max(data=tpSet, title=title, months=months, splitPlot=True, dir=mapDir).to_netcdf(path)
        tpSet.compute[name] = path
