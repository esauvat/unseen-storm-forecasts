...
def boundaries(
    ...
    s, n, w, e = bound_values[size]  # Accessing the wanted values for the boundValues

    latS = max(
        s,
        data.coords['latitude'][-1])  # Correcting each value if necessary to make sure the map
    latN = min(
        n,
        data.coords['latitude'][0])  # does not outrange the data
    lonW = max(
        w,
        data.coords['longitude'][0])
    lonE = min(
        e,
        data.coords['longitude'][-1])

    return np.array(
        [lonW, lonE, latS, latN]), (latS + latN) / 2, (lonW + lonE) / 2
...
def select_area(
    ...
    iLonW = np.where(
        np.isclose(
            data['longitude'],
            lonW))[0][0]  # Getting the index for each of the extreme
    iLonE = np.where(
        np.isclose(
            data['longitude'],
            lonE))[0][0]  # values of the latitude and longitude
    iLatS = np.where(
        np.isclose(
            data['latitude'],
            latS))[0][0]
...
def nb_to_numpy_date(
        nb: int,
        year: str
) -> np.datetime64:
    return np.datetime64(
        year + '-01-01') + np.timedelta64(
        nb,
        'D')
...
def get_day_number(
    ...
    year = np.datetime_as_string(
        date.astype(
            'datetime64[Y]'))
    return (date - np.datetime64(
        year + '-01-01')).astype(
        int)
...
def reindex_hindcast(
    ...
    da = da.stack(
        fullTime=["hdate", "time"])  # Combining the 2 time related coordinates
    newIndexes = []
...
def pears_mean(
    ...
    corr_mat = np.corrcoef(
        arr)
    return (corr_mat - np.identity(
        corr_mat.shape[0])).mean()


def pears_full(
        arr
):
    correl_mat = ma.corrcoef(
        ma.masked_array(
            arr))
    norm = np.linalg.norm(
        correl_mat)
