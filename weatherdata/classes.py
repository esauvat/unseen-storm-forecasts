    ...
    for loc in pathListToData:  # Loop over all the addresses passed as arguments
        if loc.endswith('/'):  # Check if the address is a directory
            ...
            for fileLoc in txtAsList:
                ...
                pathsToFiles[(
                    type, filename[
                          :-3])] = fileLoc  # Create the key-value instance (NB: We remove the .nc in the file name)
...
class Weatherset:
    ...
    def __init__(
        ...
        self.pathsToFiles = get_paths(
            pathListToData,
            self.resolution)  # Dict of (fileType,fileName) keys with pathToFile values
        ...
        sample = xr.open_dataarray(
            self.pathsToFiles[self.fileList[0]])  # Openning any data file to acces information like the coordinates
        ...
        if (not self.resolution == '0.5') and '0.5x0.5' in self.fileList[0][
            ...
            nbLat = 2 * len(
                sample[
                    'latitude'].values) - 1  # If so, coordinates resolution is twice as much as in the sample file
            nbLon = 2 * len(
                sample[
                    'longitude'].values) - 1  # to fit with all format in the dataset (through reindexing for the 0.5 resolution files)
        else:
    ...
    def open_data(self, key: tuple[str, str]) -> xr.DataArray:
        ...
        if self.reanalysis:
            da = da.drop_vars(
                names="number",
                errors="ignore")  # If the dataset is a reanalysis' one, remove the "number" dimension if it exists
        ...
        if (
                not self.resolution == '0.5') and '0.5' in fileName:  # Checking if the data is with 0.5 resolution while the dataset is with 0.25 one
            da = da.reindex(
    ...
    def compute_time_max(self, **kwargs: Any) -> xr.DataArray:
        ...
        for key in self.fileList:
            ...
            if meanSpan:
                data = mean_over_time(
                    data,
                    meanSpan)  # Turn the data into means, did at this time to avoid most edge effects, but takes more time.
            ...
            if data['time'].shape != (0,):  # Check if the previous selection is not empty
                max_datasets.append(
                    data.max(dim='time').expand_dims(
                        { 'ref':[key] }))  # If not, compute the maximum and add it the tho max_datasets list
