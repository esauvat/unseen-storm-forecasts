...
def draw_map(
    ...
    coordsRange = kwargs.get(
        'mapSize',
        "largeNo"
    )  # The boundValues of the map in terms of geographic coordinates (default is all center and south of Norway)
    figSize = kwargs.get(
        'figSize',
        "medium")
    extent = kwargs.get(
    ...
    if 'time' in data.coords:
        nbMaps = len(
            data['time'].values)  # Set the number of subplots and the grid dimension to fit the DataArrays
    else:
        nbMaps = 1
    nbRows, nbColumns = utils.mosaic_split(
        nbMaps)

    boundaries = utils.boundaries(
        data,
        size=coordsRange)[0]  # Compute the map's boundValues to fit the dataset
    fig, axis = geo.map(
        nbRows,
        nbColumns,
        nbMaps,
        figSize,
        boundaries
    )  # Create the map with proper size and geographical background
    fig, axis = geo.showcase_data(
        data,
        boundaries,
        fig,
        axis,
        nbMaps,
        extent=extent)  # Plot the data

    if len(
            times) != 0:
        for i in range(
                nbMaps):  # If years have been specified
            axis[i].set_title(
                times[i])  # Put the year as title for each subplot

    if title:
        fig.suptitle(
            title)  # If specified, set the figure title
...
def map_of_max(
    ...
    months = kwargs.get(
        'months',
        [])  # Get the list of months ov which to compute the max
    splitYears = kwargs.get(
        'splitYears',
        False
    )  # Choose if you want to plot each year in the years list, can't be used with months
    years = kwargs.get(
        'years',
        [])  # Get the list of years on which to compute the max
    timeSpan: int = kwargs.get(
        'timeSpan',
        0)  # And the number of years each sample last
    title = kwargs.get(
        'title',
        '')  # Title of the map (plt variable)
    coordsRange = kwargs.get(
        'mapSize',
        "largeNo"
    )  # The boundValues of the map in terms of geographic coordinates (default is all center and south of Norway)
    figSize = kwargs.get(
        'figSize',
        "medium")  # Size of the plt figure
    directory = kwargs.get(
        'dir',
        '')  # Directory to store the map, is None the map is just shown
    splitPlot = kwargs.get(
        'splitPlot',
        False)

    assert len(
        months) == 0 or not splitYears, " Can't plot annual and monthly at the same time "
        ...
        for y in years:
                    ...
                    i) for i in
                               range(
                                   int(
                                       y),
                                   int(
                                       y) + timeSpan)]  # Create the list of years on which compute the max
                data_max = data.compute_time_max(
                    yearSelec=yearsSample)  # And create the max DataArray
            else:
                data_max = data.compute_time_max(
                    yearSelec=[y]
                )  # If not just create the max DataArray for the specified year
            data_max = data_max.expand_dims(
                {
                    "time":[
                        y]
                }
            )  # Create a time dimension with label the specified year's first day to use as alignment when
            # concatenating
            ...
            dataarrays.append(
                data_max.transpose(
                    ...
                    "longitude"))
            data_max.close()
        max_simulated = xr.concat(
            dataarrays,
            dim="time")
        del dataarrays
    elif len(
            months) != 0:  # Second case is when a set of months is specified
        dataarrays = []
        for m in months:
            if type(
                    m) is list:
    ...
    if not splitPlot:
        ...
        if directory:  # If a directory is specified, we want to save the map in it, hence we create the map's file name
            timeReference, resReference = '', '_all-resolution'  # Initiate a time and resolution reference to insert
            # in the name
            timeRangeReference = '_all-time'
            if not splitYears and len(
                    months) != 0 and len(
                    years) != 0:
            ...
            fig.savefig(
                directory + 'tp24-max' + resReference + timeReference + timeRangeReference + '.png')
...
def combine_4_files_ha(
    ...
    hres = tpSet.open_data(
        keys[0])
    lres = tpSet.open_data(
        keys[1])
    forecast = xr.concat(
        [hres, lres],
        dim='time',
    ...
    del forecast, hindcast, hres, lres
